...
const ArtistCard = ({ img, name }) => (
  <div className="flex flex-col h-[300px] w-[150px] justify-center">
    <img className="h-[150px] rounded-[50%] w-[150px]" src={img} alt={name} />
    ...
    <div className="flex bg-black min-h-screen p-2 gap-[10px]">
      ...
      <main className="flex flex-col bg-zinc-900 rounded-md p-4 overflow-y-auto h-screen">
        ...
        <div className="mt-16 flex flex-col">
          <h1 className="font-bold text-2xl text-white">Popular Artists</h1>
          <div className="flex w-full gap-6">
            {artists.map((item, idx) => (
              <ArtistCard key={idx} {...item} />
            ))}
          </div>
        </div>
      </main>
